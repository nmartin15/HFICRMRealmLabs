...
export function PipelineBoard({ track }: { track: PipelineBoardTrack }) {
  const router = useRouter();
  const href = PIPELINE_BOARD_HREFS[track];
  ...
  const load = useCallback(async () => {
    const data = await api<AllocationBoardResponse>(href);
    setBoard(data);
  }, [href]);
  ...
  async function moveCard(cardId: string, stage: AllocationOpenStage) {
    ...
    try {
      ...
      const next = await api<AllocationBoardResponse>(href);
      setBoard(next);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <nav aria-label="Pipeline track" className="flex flex-wrap items-center gap-1">
          {PIPELINE_BOARD_TRACKS.map((item) => (
            <Link
              key={item}
              href={PIPELINE_BOARD_HREFS[item]}
              aria-current={item === track ? "page" : undefined}
              className={cn(
